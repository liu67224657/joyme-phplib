<?php
/**
 * Description of OSS(Open Storage Services) PHP SDK
 * 阿里云云存储 PHP SDK
 *
 * @Author gradydong
 * @Copyright joyme.com
...
use Joyme\aliy\Aliy_Request;
use Joyme\aliy\Aliy_Response;
use Joyme\aliy\Aliy_Mimetypes;
use Joyme\aliy\OSSUtil;
use Exception;
...
class Aliy_Utils
{
    ...
    const DEFAULT_OSS_ENDPOINT = 'oss-cn-beijing.aliyuncs.com';

    const OSS_VERSION = '1.1.7';
    const OSS_NAME = 'aliyun-oss-sdk-php';
    ...
    protected $BucketConfig = array(
        'aliytestpro' => array(
            'access_id'=>'m2LJu94lrAKPMGBm',
            'access_key'=>'jO3aBvvxQKfBoBEHXadiLhG0YFi8OJ'
        ),
        'aliypicpro' => array(
            'access_id'=>'m2LJu94lrAKPMGBm',
            'access_key'=>'jO3aBvvxQKfBoBEHXadiLhG0YFi8OJ'
     ...
     */
    public function init($bucket,$hostname = NULL, $security_token = NULL)
    {
        if ($hostname) {
        ...
        //初始化访问id和访问key
        $AliyKey = $this->BucketConfig[$bucket];
        if($AliyKey){
            $this->access_id = $AliyKey['access_id'];
            $this->access_key = $AliyKey['access_key'];
            return true;
        }else{
            return false;
        }
    ...
    {
        $this->precheck_common($bucket, NULL, $options, false);
        $this->precheck_bucket($target_bucket, 'TargetBucket不允许为空');
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_PUT;
        $options[self::OSS_OBJECT] = '/';
        $options[self::OSS_SUB_RESOURCE] = 'logging';
    ...
    {
        $this->precheck_common($bucket, NULL, $options, false);
        OSSUtil::is_empty($index_document, 'IndexDocument不允许为空');
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_PUT;
        $options[self::OSS_OBJECT] = '/';
    ...
    {
        $this->precheck_common($bucket, $object, $options);
        $AliyKeySrc = $this->init($bucket);
        if(!$AliyKeySrc){
            return '请检查bucket是否正确';
        }
        //内容校验
    ...
    {
        $this->precheck_common($bucket, $object, $options);
        OSSUtil::is_empty($file, '上传文件路径为空');
        $AliyKeySrc = $this->init($bucket);
        if(!$AliyKeySrc){
            return '请检查bucket是否正确';
        }
        ...
        if (!file_exists($options[self::OSS_FILE_UPLOAD])) {
            throw new Exception($options[self::OSS_FILE_UPLOAD] . '文件不存在');
        }
    ...
    public function copy_object($from_bucket, $from_object, $to_bucket, $to_object, $options = NULL)
    {
        ...
        $AliyKeySrc = $this->init($to_bucket);
        if(!$AliyKeySrc){
            return '请检查bucket是否正确';
        }
    ...
    public function delete_objects($bucket, $objects, $options = null)
    {
        ...
        if (!is_array($objects) || !$objects) {
            throw new Exception('The ' . __FUNCTION__ . ' method requires the "objects" option to be set as an array.');
        }
    ...
    public function init_multipart_upload($bucket, $object, $options = NULL)
    {
        ...
        if (!$res->isOK()) {
            throw new Exception('Init multi-part upload failed...');
        }
     ...
     * @param string $object (Required) Object名称
     * @param string $upload_id (Required) uploadId
     * @param array | string xml $parts 可以是一个上传成功part的数组，或者是一个Aliy_Reponse对象
     * @param array $options (Optional) Key-Value数组
     * @return Aliy_Response
     */
    public function complete_multipart_upload($bucket, $object, $upload_id, $parts, $options = NULL)
    {
        ...
        if (is_string($parts)) {
        ...
        } else if ((is_array($parts) || $parts instanceof Aliy_Response)) {
            ...
            } elseif ($parts instanceof Aliy_Response) {
    ...
    public function create_mpu_object($bucket, $object, $options = null)
    {
        ...
        foreach ($response_upload_part as $i => $response) {
            ...
            if (!$upload_part_result) {
                throw new Exception('any part upload failed, please try again');
            }
            $upload_parts[] = array(
                'PartNumber' => ($i + 1),
    ...
    public function create_mtu_object_by_dir($bucket, $dir, $recursive = false, $exclude = ".|..|.svn", $options = null)
    {
        ...
        //判断是否目录
        if (!is_dir($dir)) {
            throw new Exception($dir . ' is not a directory, please check it');
        }
    ...
    public function batch_upload_file($options = NULL)
    {
        ...
        //判断是否目录
        if (!is_dir($directory)) {
            throw new Exception($directory . ' is not a directory, please check it');
        }
     ...
     * @throws Exception
     */
    private function precheck_bucket($bucket, $err_msg = 'Bucket不允许为空')
    {
    ...
    {
        OSSUtil::is_empty($object, 'Object不允许为空');
    }
    ...
    {
        if (!isset($options[$param])) {
            throw new Exception('The `' . $param . '` options is required in ' . $func_name . '().');
        }
    }
    ...
    private function get_mime_type($object)
    {
        ...
        $mime_type = Aliy_Mimetypes::get_mimetype(strtolower($extension));
        return $mime_type;
    }
    ...
    public function auth($options)
    {
        ...
        if (!(('/' == $options[self::OSS_OBJECT]) && ('' == $options[self::OSS_BUCKET]) && ('GET' == $options[self::OSS_METHOD])) && !OSSUtil::validate_bucket($options[self::OSS_BUCKET])) {
            throw new Exception('"' . $options[self::OSS_BUCKET] . '"未通过Bucket名称规则校验');
        }
        //验证Object
        if (isset($options[self::OSS_OBJECT]) && !OSSUtil::validate_object($options[self::OSS_OBJECT])) {
            throw  new Exception($options[self::OSS_OBJECT] . '未通过Object名称规则校验');
        }
        //Object编码为UTF-8
        ...
        if (isset($options[self::OSS_HEADERS][self::OSS_ACL]) && !empty($options[self::OSS_HEADERS][self::OSS_ACL])) {
            if (!in_array(strtolower($options[self::OSS_HEADERS][self::OSS_ACL]), self::$OSS_ACL_TYPES)) {
                throw new Exception($options[self::OSS_HEADERS][self::OSS_ACL] . ':ACL不在允许范围,目前仅允许(private,public-read,public-read-write三种权限)');
            }
        }
        ...
        //创建请求
        $request = new Aliy_Request($this->request_url);
        ...
        $data = new Aliy_Response($response_header, $request->get_response_body(), $request->get_response_code());
